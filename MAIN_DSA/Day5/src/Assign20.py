...
class Library:
    ...
    def sort_item_list_by_author(self,new_item_list):
        new_item_list.sort(key=lambda i:i.get_author_name().replace(" ",""), reverse=False)
    ...
    def sort_items_by_published_year(self):
        self.__item_list.sort(key=lambda i:(i.get_published_year(),i.get_author_name().replace(" ","")))
